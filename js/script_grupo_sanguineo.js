$( document).ready(function(){ 
    ...
    $("#enviarInsert").click(function(){
        $.post("../controlador/grupo_sanguineo/controlador_grupo_sanguineo_insertar.php",{nombre_grupo_sanguineo: $('#insertar #nombre_grupo_sanguineo').val()}, function(resp){
   $("#insertar #resultado").html(resp);
    ...
    $("#enviarUpdate").click(function(){
        $.post("../controlador/grupo_sanguineo/controlador_grupo_sanguineo_modificar.php",{id: $('#idModificar option:selected').text(), nombre: $('#modificar #nombre').val()}, function(resp){
...
function cargarListas(){
      //Limpia los select
    $('#eliminaNombreGrupoSanguineo')
    .empty();
    $('#eliminaIdGrupoSanguineo')
    .empty();
    $('#idModificar')
    .empty();
    ...
    $.post("../controlador/grupo_sanguineo/controlador_grupo_sanguineo_consultar.php", function(resp){
        var parse = JSON.parse(resp);
        for(var x=0 ; x < (parse.listaopciones.length-1); x++){
            $("#eliminaIdGrupoSanguineo").append( '<option value="'+parse.listaopciones[x].id+ '">'+parse.listaopciones[x].id+' </option>');
            $("#idModificar").append( '<option value="'+parse.listaopciones[x].id+ '">'+parse.listaopciones[x].id+' </option>');
             $("#eliminaNombreGrupoSanguineo").append( '<option value="'+parse.listaopciones[x].nombre+ '">'+parse.listaopciones[x].nombre+' </option>');
        }
    ;});
}
